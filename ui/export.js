...
class ExportManager {
  ...
  _buildSinglePDF(doc, r) {
    ...
    const aiScore  = aiData.aiScore || 0;
    const humanScore = 100 - aiScore;
    const aiConf   = aiData.confidence || 0;
    const confLabel = aiConf > 80 ? 'High' : (aiConf > 50 ? 'Medium' : 'Low');
    ...
    const verdictIcon = isAI ? 'AI-GENERATED' : 'HUMAN-WRITTEN';
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(16);
    doc.setTextColor(...verdictClr);
    doc.text(verdictIcon, ML + 6, y + 11);
    
    const modelSig = aiData.modelSignature?.model || '';
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.text(modelSig ? `Likely Generated By: ${modelSig}` : `Confidence: ${confLabel} (${aiConf}%)`, ML + 6, y + 20);
    
    // Right side: big % score
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(22);
    doc.setTextColor(...verdictClr);
    doc.text(`${isAI ? aiScore : humanScore}%`, PW - ML - 20, y + 12, { align: 'right' });
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
    ...
    doc.text(`Human Score: ${humanScore}%   |   AI Score: ${aiScore}%   |   Confidence: ${confLabel} (${aiConf}%)`, ML, y);
    y += 12;
    ...
    // Signal bars from real data
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(12);
    doc.setTextColor(51, 51, 51);
    doc.text('Detection Signals', ML, y);
    y += 8;

    const signals = [
        { label: 'Fast-DetectGPT Curvature',    value: aiData.probabilityCurvature?.aiLikelihood || 0,  raw: `curvature: ${(aiData.probabilityCurvature?.curvature||0).toFixed(2)}` },
        { label: 'Stylometric Inconsistency',   value: aiData.stylometric?.varianceScore || 0,           raw: `variance: ${(aiData.stylometric?.varianceScore||0).toFixed(2)}` },
        { label: 'Sentence Uniformity',         value: aiData.signals?.sentenceLengthUniformity || 0,    raw: `score: ${Math.round((aiData.signals?.sentenceLengthUniformity||0)*100)}%` },
        { label: 'Transition Word Density',     value: aiData.signals?.transitionWordDensity || 0,       raw: `score: ${Math.round((aiData.signals?.transitionWordDensity||0)*100)}%` },
        { label: 'Perplexity Proxy',            value: aiData.signals?.perplexityProxy || 0,             raw: `score: ${Math.round((aiData.signals?.perplexityProxy||0)*100)}%` },
        { label: 'Zipf Law Deviation',          value: aiData.signals?.zipfDeviation || 0,               raw: `score: ${Math.round((aiData.signals?.zipfDeviation||0)*100)}%` },
        { label: 'Long Word Ratio',             value: aiData.signals?.longWordRatio || 0,               raw: `score: ${Math.round((aiData.signals?.longWordRatio||0)*100)}%` },
        { label: 'Structural Repetition',       value: aiData.signals?.structuralRepetition || 0,        raw: `score: ${Math.round((aiData.signals?.structuralRepetition||0)*100)}%` },
    ];
    ...
    signals.forEach(sig => {
        ...
        doc.setFontSize(9);
        doc.setTextColor(80, 80, 80);
        doc.text(sig.label, ML, y);
        doc.setTextColor(130, 130, 130);
        doc.text(sig.raw, PW - ML, y, { align: 'right' });
        y += 4;
        
        // Bar background
        doc.setFillColor(230, 230, 230);
        doc.roundedRect(ML, y, barW, 4, 1, 1, 'F');
        
        // Bar fill — clamp to 0-1 range
        const pct = Math.min(1, Math.max(0, sig.value));
        const fillR = Math.round(pct > 0.6 ? 220 : (pct > 0.3 ? 245 : 34));
        const fillG = Math.round(pct > 0.6 ? 38  : (pct > 0.3 ? 158 : 197));
        const fillB = Math.round(pct > 0.6 ? 38  : (pct > 0.3 ? 11  : 94));
        doc.setFillColor(fillR, fillG, fillB);
        if (pct > 0) doc.roundedRect(ML, y, barW * pct, 4, 1, 1, 'F');
        
        // Percentage text
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(8);
        doc.setTextColor(80, 80, 80);
        doc.text(`${Math.round(pct * 100)}%`, ML + barW + 3, y + 3.5);
        y += 10;
    });

    // Model signature detail if available
    if (aiData.modelSignature?.model) {
        y += 4;
        doc.setDrawColor(220, 220, 220);
        doc.setLineWidth(0.4);
        doc.line(ML, y, PW - ML, y);
        ...
        doc.setFontSize(11);
        doc.setTextColor(51, 51, 51);
        doc.text('Model Signature Match', ML, y);
        y += 7;
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
        doc.setTextColor(80, 80, 80);
        doc.text(`Detected Model: ${aiData.modelSignature.model}`, ML, y); y += 6;
        doc.text(`Signature Confidence: ${Math.round((aiData.modelSignature.confidence||0)*100)}%`, ML, y); y += 6;
        if (aiData.modelSignature.evidence?.length) {
            const evidenceStr = 'Evidence: ' + aiData.modelSignature.evidence.slice(0,3).join(', ');
            const evLines = doc.splitTextToSize(evidenceStr, CW);
            doc.text(evLines, ML, y);
            y += evLines.length * 5;
        }
    }
